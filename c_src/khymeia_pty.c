/*
 * khymeia-pty — runs one command on a real pseudo-terminal and relays it
 ...
 * Khymeia owns this helper — the chain of supervision is unbroken.
 *
 ...
 *
 * Closing stdin kills the child: if Khymeia dies, no harness is left behind.
 */
...
/*
 * Reap the child, keeping its exit status: it is what Khymeia reports, so it
 ...
 * outlives Khymeia, so it must never be the thing that hangs: every wait is
...
/* One framed message from Khymeia. Returns 0 on EOF, -1 on error. */
...
int main(int argc, char *argv[]) {
  if (argc < 2) {
    fprintf(stderr, "usage: khymeia-pty <command> [args...]\n");
    return 64;
  }
  ...
  if (child < 0) {
    perror("khymeia-pty: forkpty");
    return 71;
  }

  if (child == 0) {
    execvp(argv[1], &argv[1]);
    perror("khymeia-pty: exec");
    _exit(127);
  }
  ...
  for (;;) {
    ...
    if (ready > 0 && stdin_open && FD_ISSET(STDIN_FILENO, &fds)) {
      ...
      if (status < 0) break;
      if (status == 0) {
        /* Khymeia is gone: kill the harness and stop, there is no reader left. */
        stdin_open = 0;
        reap_child(1);
        break;
      }
