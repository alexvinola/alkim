defmodule Alkim.Harness.Capabilities do
  @moduledoc """
  ...
  assuming every harness behaves the same.
    ...
    * `:models` — a fixed, closed list of models, or `:unknown` when the CLI
      ...
      `c:Alkim.Harness.list_models/1` and are cached by discovery.
    ...
    * `:read_only_mode` — the permission mode that prevents the harness from
      ...
      CLI's own permission system) or `:none` (Alkim cannot guarantee it).
