defmodule Alkim.Harness.Codex do
  @moduledoc """
  ...
  ## Limitations
    ...
    * `codex exec` refuses to run outside a Git repository unless
      `--skip-git-repo-check` is given. Alkim does not pass it: that check
  ...
  so Alkim cannot name the conversation up front. Resuming therefore uses
  ...
  @azure_key_env "ALKIM_AZURE_OPENAI_API_KEY"
  ...
  defp provider(%{kind: :azure_openai, settings: s, secret: secret}) do
    args =
      config("model_provider", "alkim_azure") ++
        config("model_providers.alkim_azure.name", "Azure OpenAI (Alkim)") ++
        config("model_providers.alkim_azure.base_url", String.trim_trailing(s["base_url"], "/")) ++
        config("model_providers.alkim_azure.env_key", @azure_key_env) ++
        config("model_providers.alkim_azure.wire_api", "responses")
  ...
  defp provider(%{kind: :bedrock, settings: s, secret: secret}) do
    ...
    {args, Alkim.Harness.Claude.aws_credentials(secret, profile)}
  end
