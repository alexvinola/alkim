defmodule Alkim.Harness.Discovery do
  @moduledoc """
  ...
  are broadcast on the `"harnesses"` topic of `Alkim.Runtime.EventBus`.
  ...
  Results are `t:harness/0` maps:
      ...
      %{id: :claude, name: "Claude Code", status: :available,
        ...
        adapter: Alkim.Harness.Claude, capabilities: %Capabilities{},
        models: [%{id: "opus", name: "opus", description: "..."}]}

  `models` are the ones the installed CLI reports (see
  `c:Alkim.Harness.list_models/1`), or the adapter's fixed list; `[]` when
  neither is available.

  `status` is `:available` (installed and integrated), `:not_installed`, or
  `:no_adapter` (installed, but Alkim has no integration yet).
  """

  use GenServer

  alias Alkim.Harness
  alias Alkim.Harness.Executable
  ...
  @type harness :: %{
          ...
          adapter: module() | nil,
          capabilities: Alkim.Harness.Capabilities.t() | nil,
          models: [Alkim.Harness.model()]
        }
