defmodule Alkim.Harness.Executable do
  @moduledoc """
  Locates harness executables and builds the `PATH` harnesses run with.

  When Alkim runs as a daemon (`brew services`, launchd, systemd) it does not
  ...
  directories in `ALKIM_EXTRA_PATH`.

  A specific binary can always be pinned with `ALKIM_<ID>_BIN`, e.g.
  `ALKIM_CLAUDE_BIN=/opt/tools/claude`.
  """
  ...
  @doc "Finds `name` for harness `id`, honouring `ALKIM_<ID>_BIN`."
  @spec find(atom(), String.t()) :: {:ok, String.t()} | :not_found
  def find(id, name) do
    case System.get_env("ALKIM_#{id |> Atom.to_string() |> String.upcase()}_BIN") do
  ...
  def search_path do
    ...
    extra = String.split(System.get_env("ALKIM_EXTRA_PATH", ""), ":", trim: true)
