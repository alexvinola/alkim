defmodule Alkim.Projects do
  ...
  alias Alkim.Projects.Project
  alias Alkim.Repo
  alias Alkim.Sessions.SessionRecord
  alias Alkim.Workflow.Run
  ...
  def active_counts do
    sessions =
      Alkim.Runtime.list_live()
      |> Enum.reject(&Alkim.Session.terminal?(&1.status))
      |> Enum.map(& &1[:project_id])

    workflows =
      Alkim.Workflow.list_recent(50)
      |> Enum.filter(&Alkim.Workflow.Run.active?/1)
      |> Enum.map(& &1.project_id)

    terminals =
      Alkim.Terminals.list_recent(50)
      |> Enum.filter(&Alkim.Terminals.Terminal.live?/1)
      |> Enum.filter(&Alkim.Terminals.alive?(&1.id))
      |> Enum.map(& &1.project_id)
