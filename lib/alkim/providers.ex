defmodule Alkim.Providers do
  ...
  OpenAI). See `Alkim.Providers.Profile`.
  ...
  defp fetch_secret(%Profile{credential: :env, credential_env: var}) do
    case System.get_env(var) do
      ...
      _ -> {:error, "environment variable #{var} is not set in Alkim's environment"}
    end
  end
  ...
  defp credential_checks(%Profile{credential: :env, credential_env: var}) do
    if System.get_env(var) in [nil, ""],
      do: [
        {:warn,
         "#{var} is not set in Alkim's environment (a brew services daemon does not see your shell's variables)"}
      ],
      else: [{:ok, "#{var} is set"}]
  end
