defmodule Alkim.Providers.Keychain do
  @moduledoc """
  ...
  generic passwords with service `alkim`.
  """
  ...
  def put(account, secret) do
    with true <- available?() || {:error, :unavailable},
         true <- Alkim.Providers.Secrets.valid_secret?(secret) || {:error, :invalid_secret} do
      port =
        Port.open({:spawn_executable, @security}, [:binary, :exit_status, :hide, args: ["-i"]])

      Port.command(
        port,
        ~s(add-generic-password -U -s #{@service} -a #{account} -l "Alkim provider" -w "#{secret}"\n)
      )
