defmodule Alkim.Providers.Secrets do
  @moduledoc """
  Where provider credentials live when they are not ambient.

  The default backend is the macOS Keychain (`Alkim.Providers.Keychain`).
  Alkim only stores *which* item to read; the value is fetched when a turn
  ...
  def backend, do: Application.get_env(:alkim, :secrets_backend, Alkim.Providers.Keychain)
