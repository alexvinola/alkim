defmodule Alkim.Session do
  @moduledoc """
  Runtime snapshot of a session, as returned by `Alkim.Runtime`.

  The authoritative copy lives inside the session's own process
  (`Alkim.Runtime.SessionServer`); this struct is just data handed out to
  callers. Persistent history lives in `Alkim.Sessions.SessionRecord`.
