defmodule Alkim.Workflow do
  ...
  *roles* (implementer, advisor, auditor) are played by locally installed
  ...
  Alkim does not decide which agent is better. The user maps
  `role → harness/model` (with capability-tier defaults); Alkim handles
  execution, coordination, state, handoffs, supervision and observability;
  the harnesses do the reasoning, coding and tool use.

  Everything goes through `Alkim.Runtime`: no harness ever invokes another
  one directly.
  """
  ...
  alias Alkim.Workflow.{Definition, Presets, Role, Run, Store}
  ...
  `attrs` (string or atom keys):
    ...
    * `workflow` — preset name (default `"coding-with-audit"`), or
      `definition` — a map accepted by `Alkim.Workflow.Definition.from_map/1`;
  ...
  def start(attrs, opts \\ []) do
    ...
    with {:ok, definition} <- definition(attrs),
      ...
      with {:ok, run} <- Store.insert_run(run),
           {:ok, _pid} <-
             Alkim.Workflow.Supervisor.start_workflow(
  ...
  @doc """
  Consults a consultant role (`:advisor`) on behalf of a run. Alkim
  ...
  @spec get(String.t()) :: {:ok, Run.t(), [Alkim.Workflow.Step.t()]} | :error
  def get(id) do
  ...
  def alive?(id), do: Registry.lookup(Alkim.Workflow.Registry, id) != []
  ...
  defp assign_role(role_id, choice, model, mode, definition, workspace, task) do
    {harness, profile_id} = Alkim.Providers.parse_choice(choice)

    with {:ok, adapter} <- Alkim.Harness.fetch_adapter(to_string(harness)),
        ...
        {:ok, params} ->
          ...
          role = %{
            ...
            | model: params.model,
              ...
              provider: params.profile && Alkim.Providers.Profile.label(params.profile)
          }

          note =
            if role.enforcement == :none,
              do:
                "#{role_id}: #{available.name} has no read-only mode; Alkim cannot guarantee it will not modify files"
  ...
  defp advisor_note(%{advisor: _, implementer: %Role{harness: harness}}) do
    {:ok, adapter} = Alkim.Harness.fetch_adapter(harness)
  ...
  defp call(id, message, timeout \\ 15_000) do
    GenServer.call(Alkim.Workflow.Server.via(id), message, timeout)
  catch
