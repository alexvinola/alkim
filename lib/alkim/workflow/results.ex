defmodule Alkim.Workflow.StepResult do
  @moduledoc """
  ...
  and after the step (see `Alkim.Git`); it is `nil` when the
  workspace is not a git repository — Alkim does not guess.
  """
...
defmodule Alkim.Workflow.Finding do
...
defmodule Alkim.Workflow.AuditResult do
  @moduledoc """
  A reviewer's verdict. `:unparseable` means the reviewer did not return a
  verdict Alkim could read; the workflow then asks a human instead of
  assuming anything.
  """
  alias Alkim.Workflow.Finding
