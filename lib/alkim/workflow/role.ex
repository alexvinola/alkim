defmodule Alkim.Workflow.Role do
  @moduledoc """
  ...
  **kind**, which decides how Alkim treats the step:
  ...
  in `config :alkim, :workflow_tiers` or `ALKIM_TIER_<NAME>=harness[:model]`.
  Alkim never decides which model is "better".
  """
  ...
  defstruct [
    ...
    :permission_mode,
    :tier,
    # Optional provider profile (see Alkim.Providers) and its label.
    :profile_id,
    :provider,
  ...
  """
  def tiers, do: Application.get_env(:alkim, :workflow_tiers, %{})
  ...
  defp choice(harness, profile_name) do
    case Enum.find(Alkim.Providers.list(), &(&1.name == profile_name and &1.harness == harness)) do
