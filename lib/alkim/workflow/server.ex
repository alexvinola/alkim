defmodule Alkim.Workflow.Server do
  @moduledoc """
  One process per workflow run. It walks the definition's steps, starts a
  harness session for each role through `Alkim.Runtime`, reacts to their
  ...
  (re-running it would repeat agent work); `Alkim.Runtime.CrashMonitor`
  records it as failed if it ever crashes.

  Everything visible is persisted (`Alkim.Workflow.Store`) before the
  ...
  alias Alkim.Runtime
  alias Alkim.Runtime.{CrashMonitor, EventBus}
  alias Alkim.Runtime.Event, as: SessionEvent
  ...
  def via(id), do: {:via, Registry, {Alkim.Workflow.Registry, id}}
  ...
  defp implementer_resumable?(state) do
    with %Role{harness: harness} <- state.roles[:implementer],
         {:ok, adapter} <- Alkim.Harness.fetch_adapter(harness) do
