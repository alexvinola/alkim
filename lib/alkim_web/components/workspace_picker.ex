defmodule AlkimWeb.WorkspacePicker do
  @moduledoc """
  ...
  (`Alkim.Workspace.browse/2`), confined to the allowed workspace roots.
  ...
  LiveView `{:workspace_selected, path}`:
      ...
      <.live_component module={AlkimWeb.WorkspacePicker} id="workspace-picker" value={@params["workspace"]} />
  """

  use AlkimWeb, :live_component

  alias Alkim.{Runtime, Workspace}
  ...
  def workspace_field(assigns) do
    ~H"""
    <div class="a-field">
      <span class="a-label">Workspace</span>
      <input type="hidden" name={@name} value={@value} id={"#{@picker}-value"} />
      <button
        type="button"
        class={["a-picker-trigger", @error && "a-picker-trigger-error"]}
        phx-click={JS.push("open", target: "##{@picker}")}
        id={"#{@picker}-trigger"}
      >
        <.icon name="hero-folder" class="size-4 a-muted" />
        <span class="a-mono a-truncate">{display(@value) || "Choose a folder…"}</span>
        <span class="a-hint" style="margin-left:auto">Browse…</span>
      </button>
    ...
    <div id={@id}>
      ...
      >
        <div
          ...
          phx-target={@myself}
        >
          <header class="a-modal-head">
            <h2 class="a-h1" id={"#{@id}-title"} style="font-size:16px">Choose a workspace</h2>
            <button
              type="button"
          ...
          <div class="a-modal-body">
            <aside class="a-picker-side">
              <div :if={@recent != []}>
                <span class="a-h2">Recent</span>
                <button
                  :for={path <- @recent}
                  type="button"
                  class="a-picker-link"
                ...
                >
                  ...
                  <span class="a-truncate">{Path.basename(path)}</span>
                </button>
              </div>
              <div>
                <span class="a-h2">Roots</span>
                <button
                  :for={root <- @roots}
                  type="button"
                  class="a-picker-link"
                ...
                >
                  ...
                  <span class="a-truncate">{display(root)}</span>
                </button>
              </div>
            </aside>

            <section class="a-picker-main">
              <nav :if={@listing} class="a-crumbs" aria-label="Path">
                <button
              ...
              >
                <input
                  ...
                  id={"#{@id}-filter"}
                />
                <label class="a-hint a-check-label">
                  <input
                    type="checkbox"
                    checked={@hidden}
              ...
              <div :if={@error} class="a-banner a-banner-error">{@error}</div>

              <ul :if={@listing} class="a-picker-list" id={"#{@id}-list"}>
                <li :if={@listing.parent}>
                  <button
                    ...
                    phx-target={@myself}
                  >
                    <.icon name="hero-arrow-uturn-left" class="size-4 a-muted" />
                ...
                <li :for={dir <- visible(@listing.dirs, @filter)}>
                  ...
                  >
                    <.icon name="hero-folder" class="size-4 a-muted" />
                    <span class="a-truncate">{dir.name}</span>
                    <span :if={dir.git} class="a-tag">git</span>
                  </button>
                </li>
                <li :if={visible(@listing.dirs, @filter) == []} class="a-empty">
                  {if @filter == "", do: "No subfolders.", else: "No folder matches “#{@filter}”."}
                </li>
                <li :if={@listing.truncated} class="a-hint" style="padding:.5rem .75rem">
          ...
          <footer :if={@listing} class="a-modal-foot">
            <span class="a-mono a-truncate" title={@listing.path}>
              {display(@listing.path)}
              <span :if={@listing.git} class="a-tag">git</span>
            </span>
            <div style="display:flex;gap:.5rem;flex:none">
              <button type="button" class="a-btn" phx-click="close" phx-target={@myself}>Cancel</button>
              <button
                type="button"
