defmodule AlkimWeb.SessionLive do
  @moduledoc """
  ...
  alias Alkim.{Harness, Runtime, Session}
    ...
    case Runtime.get_session(id) do
      {:ok, session, events} ->
        {:ok,
         socket
         |> assign(
           page_title: "#{Session.title(session)} · Alkim",
           session: session,
           ...
           has_events: events != [],
           project: Alkim.Projects.get(session.project_id),
           message: ""
         )
    ...
    <Layouts.app flash={@flash} nav={@nav} active={:sessions} project={@project}>
      <div class="a-section-head">
        <div style="min-width:0">
          <h1 class="a-h1 a-truncate">{Session.title(@session)}</h1>
          <span class="a-mono a-faint">{@session.id}</span>
          <.link
      ...
      <section class="a-section">
        <div class="a-panel">
          <dl class="a-meta" id="session-meta">
            <div>
            ...
            <div>
              <dt>Workspace</dt>
              <dd class="a-mono">{short_path(@session.workspace)}</dd>
            </div>
            ...
            <div :if={@session.exit_code}>
              <dt>Exit code</dt><dd class="a-mono">{@session.exit_code}</dd>
            </div>
            <div :if={@session.os_pid}>
              <dt>OS pid</dt><dd class="a-mono">{@session.os_pid}</dd>
            </div>
            <div :if={@session.harness_ref}>
              <dt>Harness session</dt>
              <dd class="a-mono a-truncate" title={@session.harness_ref}>{@session.harness_ref}</dd>
            </div>
          </dl>
          <div :if={@session.error} class="a-banner a-banner-error" style="margin:0 1rem 1rem">
            {@session.error}
          </div>
        </div>
      </section>
      ...
      <section class="a-section">
        <div class="a-section-head">
          <h2 class="a-h2">Activity</h2>
        </div>
        <div class="a-panel">
          <div :if={not @has_events} class="a-empty">
            <%= if live?(@session) do %>
              Waiting for output…
            <% else %>
              Activity is kept in memory only while the session process is alive.
              <div class="a-prompt" style="text-align:left;margin-top:.75rem">{@session.prompt}</div>
            <% end %>
          </div>
          <div id="activity" class="a-activity" phx-update="stream" phx-hook="FollowTail">
            <.event
  ...
  defp composer(assigns) do
    ...
    >
      ...
      <div class="a-composer-actions">
        <span class="a-hint">Resumes the harness conversation with a new turn.</span>
        <button type="submit" class="a-btn a-btn-primary" disabled={@session.status != :waiting}>
          Send message
        </button>
      </div>
    </form>
    <div :if={not @capabilities.resume and not Session.terminal?(@session)} class="a-composer a-hint">
