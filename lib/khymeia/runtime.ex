defmodule Khymeia.Runtime do
  @moduledoc """
  Public API of the Khymeia runtime.
  ...
  `Khymeia.Runtime.SessionServer`.
  """

  alias Khymeia.{Harness, Session, Sessions, Workspace}
  ...
  events.
  """
  @spec get_session(String.t()) :: {:ok, Session.t(), [Khymeia.Runtime.Event.t()]} | :error
  ...
  def recent_workspaces(limit \\ 8) do
    ...
    sessions =
      from(s in Khymeia.Sessions.SessionRecord,
        group_by: s.workspace,
        select: {s.workspace, max(s.inserted_at)}
      )

    workflows =
      from(w in Khymeia.Workflow.Run,
        group_by: w.workspace,
        select: {w.workspace, max(w.inserted_at)}
      )

    (Khymeia.Repo.all(sessions) ++ Khymeia.Repo.all(workflows))
