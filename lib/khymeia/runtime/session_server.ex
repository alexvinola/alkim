defmodule Khymeia.Runtime.SessionServer do
  @moduledoc """
  ...
  publishes every event on `Khymeia.Runtime.EventBus`.
  ...
  `Khymeia.Runtime.CrashMonitor` calls `record_crash/2`. Nothing else in the
  runtime is affected.
  ...
  alias Khymeia.{Session, Sessions}
  alias Khymeia.Harness.Capabilities
  alias Khymeia.Runtime.{CrashMonitor, Event, EventBus, OSProcess, Registry}
  ...
  ## Client API — normally reached through Khymeia.Runtime.
  ...
  defp config(key, default), do: Application.get_env(:khymeia, key, default)
end
