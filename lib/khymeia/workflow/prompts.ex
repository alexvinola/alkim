defmodule Khymeia.Workflow.Prompts do
  @moduledoc """
  The text each role receives. Kept in one place so the contract between
  Khymeia and the harnesses (the tagged blocks of `Khymeia.Workflow.Protocol`)
  ...
  alias Khymeia.Workflow.{AuditResult, Finding}

  @doc "First turn of the implementer."
  def implementer(ctx) do
    """
    You are the IMPLEMENTER in a Khymeia workflow. Another agent will
    ...
    <khymeia:ask-human>your question</khymeia:ask-human>
  ...
  def fix(ctx, %AuditResult{} = audit) do
    """
    Khymeia: the independent auditor reviewed your work (audit round #{ctx.round}) and reported:
  ...
  def advisor_answer(reason, answer) do
    """
    Khymeia: answer from the advisor (#{reason}):
  ...
  def advisor_unavailable(why) do
    """
    Khymeia: the advisor could not be consulted (#{why}).
  ...
  def advisor(ctx, reason, question) do
    """
    You are an ADVISOR consulted by a coding agent through Khymeia. Do not modify any
  ...
  def auditor(ctx) do
    """
    You are an independent AUDITOR in a Khymeia workflow. Do NOT modify any files.
    ...
    <khymeia:audit>{"status": "passed", "findings": []}</khymeia:audit>

    or, if there are issues:

    <khymeia:audit>{"status": "findings", "findings": [{"severity": "critical|high|medium|low", "title": "...", "description": "...", "file": "path or null", "line": 1}]}</khymeia:audit>
    """
  end
  ...
  defp advisor_instructions(%{advisor: %{max_calls: max, allowed_reasons: reasons}})
    ...
    <khymeia:ask-advisor reason="REASON">your question</khymeia:ask-advisor>
    and stop, where REASON is one of: #{Enum.join(reasons, ", ")}.
    Khymeia will send you the advisor's answer.
    """
  end
