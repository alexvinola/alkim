defmodule Khymeia.Workflow.Timeline do
  @moduledoc """
  The conversation-like view of a run (USER / IMPLEMENTER / ADVISOR /
  AUDITOR / KHYMEIA), projected from the persisted run and steps.
  ...
  alias Khymeia.Workflow.{AuditResult, Prompts, Run, Step}

  @type entry :: %{
          id: String.t(),
          who: :user | :implementer | :advisor | :auditor | :khymeia | atom(),
          at: DateTime.t() | nil,
          text: String.t(),
  ...
  defp entries(%Step{kind: :advisor} = s) do
    ...
    [
      ...
      case s.status do
        ...
        :failed ->
          [entry(s, "answer", :khymeia, s.completed_at, "Advisor unavailable: #{s.error}", :bad)]
  ...
  defp entries(%Step{} = s) do
    who = if s.role, do: String.to_existing_atom(s.role), else: :khymeia
  ...
  defp failure(s, :failed),
    do: [entry(s, "end", :khymeia, s.completed_at, "#{s.step} failed: #{s.error}", :bad)]

  defp failure(s, :stopped),
    do: [entry(s, "end", :khymeia, s.completed_at, "#{s.step} stopped", :muted)]
