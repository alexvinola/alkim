defmodule KhymeiaWeb.ProjectsLive do
  @moduledoc """
  ...
  import KhymeiaWeb.SessionComponents, only: [work_card: 1, short_path: 1]

  alias Khymeia.{Projects, Runtime, Workflow}
  alias KhymeiaWeb.{WorkEntry, WorkspacePicker}
  ...
  def mount(_params, _session, socket) do
    ...
    {:ok, socket |> assign(page_title: "Projects · Khymeia") |> load()}
  end
  ...
  def render(assigns) do
    ...
    <Layouts.app flash={@flash} nav={@nav} active={:projects}>
      ...
      <section class="k-section">
        <div :if={@nav.projects == []} class="k-panel k-empty">
          Add the folder you work in. Khymeia only ever runs harnesses inside it.
        </div>
        ...
        <div class="k-grid">
          <.link
            ...
            class="k-card k-card-project"
          >
            <div class="k-card-head">
              <.icon name="hero-folder" class="size-4 k-faint" />
              <span :if={@nav.active_counts[project.id]} class="k-badge">
                {@nav.active_counts[project.id]} active
              </span>
            </div>
            <p class="k-card-title">{project.name}</p>
            <p class="k-mono k-faint k-truncate">{short_path(project.path)}</p>
          </.link>
        </div>
      </section>

      <section class="k-section" id="active-work">
        <div class="k-section-head">
          <h2 class="k-h2">Running now</h2>
          <.link navigate={~p"/sessions"} class="k-link">All sessions</.link>
        </div>

        <div :if={@active == []} class="k-panel k-empty">Nothing is running.</div>
        <div class="k-grid">
      ...
      <section class="k-section" id="harnesses">
        <div class="k-section-head">
          ...
          <button class="k-btn k-btn-ghost" phx-click="rescan" phx-disable-with="Scanning…">
            Rescan
          </button>
        </div>
        <div class="k-panel k-rows">
          <div :if={@nav.harnesses == []} class="k-empty">Scanning for harnesses…</div>
          <div :for={h <- @nav.harnesses} class="k-row k-row-harness" id={"harness-#{h.id}"}>
