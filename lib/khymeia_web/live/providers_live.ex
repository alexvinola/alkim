defmodule KhymeiaWeb.ProvidersLive do
  @moduledoc """
  ...
  inference goes to the provider. Secrets are never stored by Khymeia: they
  stay ambient (AWS profile, `az login`, gcloud ADC), in an environment
  variable of the Khymeia process, or in the macOS Keychain.
  """

  use KhymeiaWeb, :live_view

  alias Khymeia.{Harness, Providers}
  ...
  def mount(_params, _session, socket) do
    {:ok, socket |> assign(page_title: "Providers · Khymeia") |> load()}
  end
  ...
  def render(assigns) do
    ...
    <Layouts.app flash={@flash} nav={@nav} active={:providers}>
      ...
      <p class="k-muted" style="max-width:48rem;margin:-.25rem 0 1.5rem">
        ...
        Khymeia never calls these providers itself and never stores secrets.
      </p>

      <section class="k-section" id="profiles">
        <div class="k-panel k-rows">
          <div :if={@profiles == []} class="k-empty">No provider profiles yet.</div>
          <div :for={p <- @profiles} class="k-provider" id={"profile-#{p.id}"}>
            <div class="k-provider-head">
              <strong>{p.name}</strong>
              <span class="k-tag">{harness_name(p.harness)}</span>
              <span class="k-tag">{Profile.kind_label(p.kind)}</span>
              <span class="k-hint">{credential_label(p)}</span>
              <span style="margin-left:auto;display:flex;gap:.4rem">
                <.link patch={~p"/providers?edit=#{p.id}"} class="k-btn k-btn-ghost">Edit</.link>
                <button
                ...
                <button
                  class="k-btn k-btn-ghost k-btn-danger"
                  phx-click="delete"
                  phx-value-id={p.id}
            ...
            <div class="k-mono k-faint k-truncate">
              {settings_summary(p)}{p.default_model && "  ·  model #{p.default_model}"}
            </div>
            <ul class="k-checks">
              <li :for={{level, text} <- @readiness[p.id]} class={"k-check-#{level}"}>
      ...
      <section class="k-section">
        <div class="k-section-head">
          <h2 class="k-h2">{if @editing, do: "Edit #{@editing.name}", else: "New profile"}</h2>
          <.link :if={@editing} patch={~p"/providers"} class="k-btn k-btn-ghost">Cancel</.link>
        </div>
        ...
        >
          <div class="k-field-row">
            <div class="k-field">
              <label class="k-label" for="profile_name">Name</label>
              <input
                id="profile_name"
                name="profile[name]"
                value={@params["name"]}
              ...
              />
              <span :if={@errors[:name]} class="k-error">{@errors[:name]}</span>
            </div>
            <div class="k-field">
              <label class="k-label" for="profile_target">Harness → provider</label>
              <select
                id="profile_target"
                name="profile[target]"
                class="k-select"
          ...
          <div :for={key <- @setting_keys} class="k-field">
            <label class="k-label" for={"profile_settings_#{key}"}>{field_label(key, @harness, @kind)}</label>
            <input
          ...
          <span :if={@errors[:settings]} class="k-error">{@errors[:settings]}</span>

          <div class="k-field">
            <label class="k-label" for="profile_default_model">{model_label(@harness, @kind)}</label>
            <input
            ...
            />
            <span :if={@errors[:default_model]} class="k-error">{@errors[:default_model]}</span>
          </div>

          <fieldset class="k-field k-fieldset">
            <legend class="k-label">Credential</legend>
            <label :for={c <- @credential_options} class="k-radio">
              <input type="radio" name="profile[credential]" value={c} checked={c == @credential} />
              {credential_option(c, @harness, @kind)}
            </label>
            <span :if={@errors[:credential]} class="k-error">{@errors[:credential]}</span>

            <div :if={@credential == :env} class="k-field">
              <input
              ...
              />
              <span class="k-hint">
                Name of a variable in Khymeia's own environment. A `brew services` daemon does not see your shell's exports.
              </span>
              <span :if={@errors[:credential_env]} class="k-error">{@errors[:credential_env]}</span>
            </div>

            <div :if={@credential == :keychain} class="k-field">
              <input
                type="password"
                name="profile[secret]"
                class="k-input k-mono"
              ...
              />
              <span class="k-hint">Stored in your macOS Keychain (service “khymeia”), never in Khymeia's database.</span>
              <span :if={@errors[:secret]} class="k-error">{@errors[:secret]}</span>
            </div>

            <div :if={@credential == :aws_keys} class="k-field" id="aws-keys">
              <input
          ...
          <div>
            <button type="submit" class="k-btn k-btn-primary" id="save-profile">
