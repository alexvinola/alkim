defmodule KhymeiaWeb.Plugs.LocalOnly do
  @moduledoc """
  Rejects HTTP requests whose `Host` header is not a loopback name.

  Khymeia controls processes with filesystem access, and binds to
  ...
  Extra host names can be allowed with `config :khymeia, :allowed_hosts`.
  """
  ...
  def call(conn, _opts) do
    if conn.host in @loopback or conn.host in Application.get_env(:khymeia, :allowed_hosts, []) do
      conn
    else
      conn
      |> put_resp_content_type("text/plain")
      |> send_resp(403, "Khymeia only answers requests addressed to localhost.\n")
      |> halt()
    end
  end
end
