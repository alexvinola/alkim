defmodule Alkim.ProvidersTest do
  ...
  describe "profiles" do
    ...
    test "Keychain secrets are stored outside the database and removed with the profile" do
      ...
      assert {:ok, @secret} = Alkim.MemorySecrets.get("profile:#{p.id}")

      # Nothing secret in the stored row.
      row = Alkim.Repo.reload(p)
      ...
      assert {:error, :not_found} = Alkim.MemorySecrets.get("profile:#{p.id}")
    end
    ...
    test "resolution reports a missing env credential clearly" do
      {:ok, p} =
        Providers.save(%{
          ...
          kind: :bedrock,
          credential: :env,
          credential_env: "ALKIM_TEST_UNSET_VAR"
        })

      assert {:error, "environment variable ALKIM_TEST_UNSET_VAR is not set" <> _} =
               Providers.resolve(p.id)

      assert [{:warn, "ALKIM_TEST_UNSET_VAR is not set" <> _}] = Providers.readiness(p)
    end
  end
  ...
  describe "AWS access keys from a form" do
    ...
    test "end to end: stored in the Keychain, delivered to the process, never exposed", %{
      ...
      refute inspect({events, Sessions.get(session.id), Alkim.Repo.reload(p)}) =~ @sak
  ...
  describe "Codex provider overrides" do
    test "Azure OpenAI uses a custom provider through -c, key only in the environment" do
      ...
      assert ~s(model_provider="alkim_azure") in args

      assert ~s(model_providers.alkim_azure.base_url="https://acme.openai.azure.com/openai/v1") in args

      assert ~s(model_providers.alkim_azure.env_key="ALKIM_AZURE_OPENAI_API_KEY") in args
      assert ~s(model_providers.alkim_azure.wire_api="responses") in args
      assert ["-m", "my-deployment"] == Enum.slice(args, Enum.find_index(args, &(&1 == "-m")), 2)
      assert env == [{"ALKIM_AZURE_OPENAI_API_KEY", @secret}]
      refute Enum.any?(args, &String.contains?(&1, @secret))
    end
  ...
  describe "sessions with a provider profile" do
    test "the profile and its secret reach the harness process, and nothing else", %{
      ...
      # The secret is nowhere Alkim keeps or publishes anything.
    ...
    test "each turn resolves the credential again", %{workspace: ws} do
      System.put_env("ALKIM_TEST_DEMO_KEY", @secret)
      ...
      {:ok, p} =
        Providers.save(%{
          ...
          kind: :demo,
          credential: :env,
          credential_env: "ALKIM_TEST_DEMO_KEY"
        })
      ...
      System.delete_env("ALKIM_TEST_DEMO_KEY")
      :ok = Runtime.send_message(session.id, "again")
      {failed, _} = await_event(session.id, :failed)
      assert failed.data.error =~ "ALKIM_TEST_DEMO_KEY is not set"
    end
    ...
    test "workflow roles can run through a provider profile", %{workspace: ws} do
      ...
      assert Alkim.Workflow.Store.get_run(run.id).roles["implementer"]["provider"] ==
