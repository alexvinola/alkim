defmodule Alkim.Runtime.SupervisionTest do
  ...
  alias Alkim.Runtime.{Registry, CrashMonitor, SessionSupervisor}
  ...
  test "a crashing session does not affect its siblings or the runtime", %{workspace: ws} do
    ...
    runtime = Process.whereis(Alkim.Runtime.Supervisor)
    ...
    assert Process.whereis(Alkim.Runtime.Supervisor) == runtime
  ...
  test "Discovery crashing does not touch sessions", %{workspace: ws} do
    ...
    old = Process.whereis(Alkim.Harness.Discovery)
    Process.exit(old, :kill)
    eventually(fn -> (pid = Process.whereis(Alkim.Harness.Discovery)) && pid != old end)
