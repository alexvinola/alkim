defmodule Alkim.TerminalsTest do
  ...
  test "a terminal belongs to the project of its workspace" do
    ...
    project = Alkim.Projects.get_by_path(workspace)
  ...
  test "output is saved to disk and survives the terminal's process" do
    ...
    assert File.regular?(Alkim.Terminals.Log.path(terminal.id))
  end
  ...
  test "saved output is readable only by its owner" do
    ...
    %{mode: mode} = File.stat!(Alkim.Terminals.Log.path(terminal.id))
  ...
  test "a conversation that cannot be resumed still leaves a usable terminal" do
    ...
    # The harness refuses and exits; Alkim says so and starts a fresh one
    # rather than handing back a terminal that died on arrival.
    assert await_output(terminal.id, "could not continue") =~ "[alkim]"
  ...
  # Alkim only offers a terminal for harnesses whose interactive mode was
  # verified; a merely detected CLI gets no invented command line.
  test "a harness Alkim has no adapter for is refused" do
