defmodule Alkim.Workflow.PureTest do
  @moduledoc "Definitions, protocol parsing, git snapshots and the timeline — no processes."
  use ExUnit.Case, async: true

  alias Alkim.Git
  alias Alkim.Workflow.{AuditResult, Definition, Presets, Protocol, Run, Step, Timeline}
  ...
  describe "Protocol" do
    test "finds advisor and human requests, last one wins" do
      assert {:ask_advisor, "security", "Is this safe?"} =
               Protocol.parse_request(
                 ~s(ok <alkim:ask-advisor reason="security">Is this safe?</alkim:ask-advisor>)
               )

      assert {:ask_advisor, "unspecified", "Q"} =
               Protocol.parse_request("<alkim:ask-advisor>Q</alkim:ask-advisor>")

      assert {:ask_human, "Public?"} =
               Protocol.parse_request(
                 "<alkim:ask-advisor>old</alkim:ask-advisor> then <alkim:ask-human>Public?</alkim:ask-human>"
               )
    ...
    test "parses audit verdicts, tolerating code fences" do
      ...
      assert %AuditResult{status: :passed} =
               Protocol.parse_audit(
                 ~s(<alkim:audit>{"status":"passed","findings":[]}</alkim:audit>)
               )

      # "findings" without any finding is a pass; unknown severities become medium.
      assert %AuditResult{status: :passed} =
               Protocol.parse_audit(
                 ~s(<alkim:audit>{"status":"findings","findings":[]}</alkim:audit>)
               )

      assert %AuditResult{findings: [%{severity: :medium}]} =
               Protocol.parse_audit(
                 ~s(<alkim:audit>{"status":"findings","findings":[{"severity":"meh","title":"x"}]}</alkim:audit>)
               )
    end
    ...
    test "never guesses a verdict" do
      ...
      assert %AuditResult{status: :unparseable} =
               Protocol.parse_audit("<alkim:audit>not json</alkim:audit>")

      assert %AuditResult{status: :unparseable} =
               Protocol.parse_audit(~s(<alkim:audit>{"status":"maybe"}</alkim:audit>))
    end

    test "strips protocol blocks from text shown to people" do
      assert Protocol.strip(~s(Done. <alkim:audit>{"status":"passed"}</alkim:audit>)) ==
               "Done."
    end
  end
  ...
  describe "Git" do
    ...
    test "outside a repository nothing is invented" do
      ...
      dir = Path.join(System.tmp_dir!(), "alkim-nogit-#{System.unique_integer([:positive])}")
      File.mkdir_p!(dir)
