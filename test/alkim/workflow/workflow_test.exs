defmodule Alkim.WorkflowTest do
  use Alkim.RuntimeCase, async: false

  alias Alkim.Workflow
  alias Alkim.Workflow.{Run, Store}
  ...
  describe "coding with audit" do
    test "implement → audit → pass completes the workflow", %{workspace: ws} do
      ...
      eventually(fn ->
        Alkim.Runtime.list_live() |> Enum.all?(&Alkim.Session.terminal?(&1.status))
      end)
    end
  ...
  describe "advisor consultations" do
    test "an implementer can consult the advisor through Alkim", %{workspace: ws} do
      run =
      ...
      eventually(fn -> Alkim.Runtime.Registry.lookup(advisor.session_id) == :error end)
  ...
  describe "failures stay contained" do
    test "an auditor crash leaves the workflow waiting, and the runtime up", %{workspace: ws} do
      ...
      assert Process.whereis(Alkim.Workflow.Supervisor)
      assert Process.whereis(Alkim.Runtime.SessionSupervisor)
    ...
    test "a crashed workflow is recorded and its agents stop", %{workspace: ws} do
      ...
      [{pid, _}] = Registry.lookup(Alkim.Workflow.Registry, run.id)
      ...
      eventually(fn ->
        not Process.alive?(agent) or
          Alkim.Runtime.get_session(step.session_id) |> elem(1) |> Map.get(:status) == :stopped
      end)
    end
