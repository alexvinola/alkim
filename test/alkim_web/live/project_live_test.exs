defmodule AlkimWeb.ProjectLiveTest do
  use AlkimWeb.ConnCase, async: false

  import Phoenix.LiveViewTest
  import Alkim.RuntimeCase, only: [workspace!: 0, start_fake!: 2, eventually: 1]

  alias Alkim.Projects
  ...
  test "the overview opens a terminal in the project's folder", %{conn: conn, project: project} do
    ...
    assert [terminal] = Alkim.Terminals.list_for_project(project.id)
    assert terminal.workspace == project.path
    assert Alkim.Terminals.alive?(terminal.id)
  ...
  test "a terminal keeps its output after its process is gone", %{conn: conn, project: project} do
    ...
    Alkim.Terminals.stop(terminal.id)
    eventually(fn -> not Alkim.Terminals.alive?(terminal.id) end)

    # Re-attaching reads the saved log, which is what survives a restart.
    {:ok, _terminal, scrollback} = Alkim.Terminals.attach(terminal.id)
  ...
  test "a page load never starts a terminal twice", %{conn: conn, project: project} do
    ...
    refute Alkim.Terminals.alive?(terminal.id)

    # Connecting does, exactly once.
    {:ok, _view, _html} = live(conn, ~p"/projects/#{project.id}/terminal?t=#{terminal.id}")
    eventually(fn -> Alkim.Terminals.alive?(terminal.id) end)
    assert [_one] = Alkim.Terminals.list_for_project(project.id)
  end
  ...
  test "deleting a terminal from the UI removes its saved output", %{
    ...
    view |> form("#open-terminal", terminal: %{harness: "fake"}) |> render_submit()
    [terminal] = Alkim.Terminals.list_for_project(project.id)
    path = Alkim.Terminals.Log.path(terminal.id)
    ...
    assert Alkim.Terminals.get(terminal.id) == nil
  ...
  describe "worktrees" do
    setup do
      path = Alkim.RuntimeCase.git_workspace!()
    ...
    test "creating one from the overview isolates it on its own branch", %{
      ...
      {:ok, view, html} = live(conn, ~p"/projects/#{project.id}")
      assert html =~ "Alkim never merges one"

      view |> form("#create-worktree", worktree: %{name: "try something"}) |> render_submit()

      assert [worktree] = Alkim.Worktrees.list_for_project(project.id)
    ...
    test "keeping leaves the branch, discarding does not", %{conn: conn, repo_project: project} do
      ...
      [drop, keep] = Alkim.Worktrees.list_for_project(project.id) |> Enum.sort_by(& &1.branch)

      view |> element("#wt-keep-#{keep.id}") |> render_click()
      view |> element("#wt-discard-#{drop.id}") |> render_click()

      assert Alkim.Worktrees.get(keep.id).status == :kept
      assert Alkim.Worktrees.get(drop.id).status == :discarded
    ...
    test "a terminal can be opened inside a worktree", %{conn: conn, repo_project: project} do
      ...
      [worktree] = Alkim.Worktrees.list_for_project(project.id)
