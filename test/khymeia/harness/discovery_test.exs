defmodule Khymeia.Harness.DiscoveryTest do
  use ExUnit.Case, async: false

  alias Khymeia.Harness.{Discovery, Executable, Fake}
  ...
  defmodule Missing do
    ...
    def capabilities, do: %Khymeia.Harness.Capabilities{}
  ...
  test "reports installed, missing and planned harnesses" do
    planned = [%{id: :ghost, name: "Ghost CLI", executable: "khymeia-definitely-not-installed"}]
    ...
    assert fake.executable =~ "khymeia-fake-harness"
  ...
  defmodule Reporting do
    ...
    def capabilities, do: %Khymeia.Harness.Capabilities{model_selection: true}
  ...
  describe "Executable" do
    test "a pinned binary overrides PATH lookup" do
      System.put_env("KHYMEIA_PINNED_BIN", "/bin/sh")
      on_exit(fn -> System.delete_env("KHYMEIA_PINNED_BIN") end)

      assert {:ok, "/bin/sh"} = Executable.find(:pinned, "whatever")
    end

    test "missing executables are not found" do
      assert :not_found = Executable.find(:nothing, "khymeia-definitely-not-installed")
    end
